...
public class DateConverter {
    ...
    @SuppressLint("SimpleDateFormat")
    private static SimpleDateFormat fullDateFormat = new SimpleDateFormat("EE, d MMMM yyyy HH:mm:ss");
    ...
    public static String dateFormat(String date) {
        Date dates = new Date();
        try {
            dates = fullDateFormat.parse(date);
        } catch (ParseException e) {
            e.printStackTrace();
        }

        date = fullDateFormat.format(dates);
